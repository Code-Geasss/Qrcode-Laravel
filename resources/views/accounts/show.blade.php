...
@if(Auth::user()->id == $account->user_id && $account->applied_for_payout != 1)
       ...
       <input type="hidden" value="{{$account->id}}" name="apply_for_payout"/>
            {!! Form::button('<i class="fa fa-credit-card"></i> Apply for payout', ['type' => 'submit', 'class' => 'btn btn-primary', 'onclick' => "return confirm('Are you sure you want to apply for payout?')"]) !!}
...
@if(Auth::user()->role_id < 3)
        ...
        <input type="hidden" value="{{$account->id}}" name="mark_as_paid"/>
            {!! Form::button('<i class="glyphicon glyphicon-ok"></i> Mark as Paid', ['type' => 'submit', 'class' => 'btn btn-success', 'onclick' => "return confirm('Are you sure you wish to confirm payout?')"]) !!}
